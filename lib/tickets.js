...
const { CORTESIA_CAP } = require('./catalog');
...
async function createTicket({ name, phone, email, dni, dob, type, amount, docType, orderId, ip, skuLabel }) {
...
async function createOrder({ id, token, buyerName, buyerEmail, ticketIds, amount, qty, skuLabel }) {
  const order = {
...
function buildTicketEmailHtml(ticket, { logoBase64 }) {
  ...
  const typeLabel = isFree ? tierTypeLabel(true) : (ticket.skuLabel || tierTypeLabel(false));
  ...
  <div style="background:#090909;padding:32px 16px;font-family:Arial,Helvetica,sans-serif;color:#F2EDE4;">
    <div style="max-width:460px;margin:0 auto;">
      ...
      <div class="pd-card" style="background:#141414;padding:36px 28px;text-align:center;">
        ...
        <p class="pd-accent" style="margin:0 0 6px;font-size:11px;letter-spacing:0.2em;text-transform:uppercase;color:#FF2800;font-weight:700;">${typeLabel}</p>
        <p class="pd-text" style="margin:0 0 24px;font-size:20px;font-weight:800;color:#F2EDE4;">${ticket.name}</p>

        <div class="pd-qr-wrap" style="background:#F2EDE4;display:inline-block;padding:16px;margin-bottom:18px;">
          <img src="cid:qrcode" alt="Código QR de tu entrada" width="200" height="200" style="display:block;" />
        </div>

        <p class="pd-text" style="margin:0 0 6px;font-family:monospace;font-size:14px;letter-spacing:1px;color:#F2EDE4;">${ticket.id}</p>
        <p class="pd-muted" style="margin:0 0 22px;font-size:12px;color:#888880;">Este código QR es válido para un solo ingreso.</p>

        <a class="pd-btn" href="${entradaUrl(ticket.token)}" style="display:inline-block;background:#FF2800;color:#0A0A0A;font-weight:700;font-size:12px;letter-spacing:0.1em;text-transform:uppercase;text-decoration:none;padding:13px 26px;margin-bottom:26px;">Ver mi entrada</a>
        ...
        <div style="border-top:1px solid rgba(255,40,0,0.25);padding-top:20px;text-align:left;">
          ...
          <p class="pd-muted" style="margin:0 0 18px;font-size:13px;color:#888880;">Válida ${tierValidityLabel(isFree)}</p>
          <p class="pd-muted" style="margin:0;font-size:12.5px;line-height:1.7;color:#888880;">
            Presenta este QR junto con tu <strong class="pd-text" style="color:#F2EDE4;">documento de identidad físico y original</strong> en la puerta. Evento exclusivo para <strong class="pd-text" style="color:#F2EDE4;">mayores de 18 años</strong>.
          </p>
        </div>
      </div>
